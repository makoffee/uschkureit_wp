...
jQuery(document).ready(function($){'use strict';
	...
	function blogPostHeight(){
		if ($(window).width() > 767) {
			...
			$('.blog-item').find('.blog-content').each(function(){
				var contentHeight  = $(this).height();
				var newSelector = $(this).closest('.blog-item');
				$(newSelector).find('.entry-meta.text-center').css('height',contentHeight);
			});

			$('.blog-single').find('.blog-content').each(function(){
				var contentHeight  = $(this).height();
				var newSelector = $(this).closest('.blog-single');
				$(newSelector).find('.entry-meta.text-center').css('height',contentHeight);
			});
		}else{
			$('.entry-meta.text-center').css('height','auto');
		}
	}
	...
	$('#main-carousel .carousel-inner').find('.item').each(function(){
		$(this).css('height',winHeight+'px');
	});

	$('#text-carousel .carousel-inner').find('.item').each(function(){
		$(this).css('height',winHeight+'px');
	});
